...
class ImprovedHierarchicalInference:
    ...
    @torch.no_grad()
    def gold_coarse_inference(
        self,
        input_ids: torch.Tensor,
        attention_mask: torch.Tensor,
        gold_coarse: torch.Tensor,
        use_constrained: bool = True,
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Oracle upper bound isolating coarse-error propagation.

        Stage 2 is conditioned on GOLD coarse labels instead of the Stage-1
        predictions, so any residual USS/LSS gap versus predicted-coarse is
        attributable to coarse-error propagation (bounded here) rather than the
        local window restriction. Stage 1 is still run so the reported coarse
        accuracy reflects the true coarse model, not the injected gold labels.
        """
        features = self.model.backbone(
            input_ids=input_ids, attention_mask=attention_mask
        ).last_hidden_state

        # Stage 1: real coarse predictions (for reporting coarse_acc only)
        coarse_preds = self.model.sample_stage1(features, attention_mask)

        # Stage 2: condition on GOLD coarse labels (the oracle)
        fine_preds = self.model.sample_stage2(features, gold_coarse, attention_mask)

        if use_constrained:
            # Constrain fine labels to the gold coarse category, mirroring the
            # predicted-coarse baseline's constraint to predicted coarse.
            fine_preds = self._apply_coarse_constraint(fine_preds, gold_coarse)
    ...
    def evaluate(
        ...
        print(f"  Gold-coarse oracle: {use_gold_coarse}")
        print(f"{'=' * 60}\n")
        ...
        for batch_idx, batch in enumerate(tqdm(dataloader, desc=f"Evaluating {split_name}")):
            ...
            if use_gold_coarse:
                coarse_preds, fine_preds = self.gold_coarse_inference(
                    input_ids, attention_mask, coarse_labels, use_constrained
                )
            elif use_mbr:
    ...
    def run_inference(self, splits, use_constrained, use_mbr, mbr_samples,
        ...
        for split in splits:
            ...
            results = self.evaluate(
                ...
                use_gold_coarse=use_gold_coarse,
...
def parse_args():
    ...
    parser.add_argument('--use_gold_coarse', action='store_true',
                        help='Oracle: condition Stage 2 on GOLD coarse labels '
                             '(bounds coarse-error propagation)')
...
def main():
    ...
    results = inference.run_inference(
        ...
        use_gold_coarse=args.use_gold_coarse,
