"""
Plot Error Breakdown by Compound Length — Hierarchical Window Format
=====================================================================
Generates visualization of USS, LSS, EM stratified by individual compound length
for hierarchical window predictions.

Usage:
    python plot_eval_by_length_hierarchical.py --data predictions.json [--output plot.png] [--max-length 15]
"""
import argparse
import json
...
def extract_compound_length(compound):
    ...
    # For hierarchical format: just use start and end indices
...
def main():
    ...
    parser.add_argument('--output', default='hierarchical_compound_length_metrics.png',
    ...
    for sample in data:
        # Hierarchical format uses different field names
        true_labels = sample.get('fine_true_labels', sample.get('true_labels', []))
        pred_labels = sample.get('fine_predictions', sample.get('predictions', []))
        
        # Process each compound in this sample
        # Use 'true_compounds' for hierarchical format
        compounds_key = 'true_compounds' if 'true_compounds' in sample else 'compounds'
        if compounds_key not in sample or not sample[compounds_key]:
            continue
            
        for compound in sample[compounds_key]:
            # Get compound length
