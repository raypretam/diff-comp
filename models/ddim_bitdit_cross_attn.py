"""
BitDit with Cross-Attention DiT for NeCTI Task
Enhances the standard BitDit by using cross-attention mechanisms
for better fusion of BERT features with diffusion denoising.
"""
...
from .dit_discrete_cross_attention import DiT as DiTCrossAttn
from random import random

__all__ = ["BitDitCrossAttn"]
...
class BitDitCrossAttn(nn.Module):
    """
    BitDit model with Cross-Attention DiT backbone.
    
    Extends BitDit by replacing the standard DiT with a cross-attention enabled DiT,
    which provides better fusion of BERT features during the denoising process.
    """
    ...
    def __init__(self,
        ...
        # Backbone: pretrained BERT or RoBERTa
        ...
        # LSTM for word-piece to word-level aggregation
        if self.add_lstm:
            self.lstm = nn.LSTM(
    ...
    def predict_start_from_noise(self, x_t, t, noise):
        return (
            ...
            extract(self.sqrt_one_minus_alphas_cumprod, t, x_t.shape) * noise
        )
    ...
    def model_predictions(self, x, t, bert_features, attention_mask, x_self_cond=None, clip_x_start=True):
        """
        Make model predictions using cross-attention DiT.
        
        Args:
            x: [bsz, len, bits] - noisy bit representation
            t: [bsz,] - diffusion timesteps
            bert_features: [bsz, len, hidden_size] - BERT contextual features
            attention_mask: [bsz, len] - attention mask
            x_self_cond: [bsz, len, bits] / None - self-conditioning signal
            clip_x_start: bool - whether to clip predictions
        
        Returns:
            ModelPrediction with pred_noise and pred_x_start
        """
        # Cross-attention DiT forward pass
    ...
    def p_sample(self, x, t, bert_features, attention_mask, x_self_cond=None, clip_denoised=True):
        ...
        model_mean, _, model_log_variance, x_start = self.p_mean_variance(
            x, batched_times, bert_features, attention_mask, x_self_cond=x_self_cond, clip_denoised=clip_denoised
        )
    ...
    def forward(self, input_ids: torch.Tensor, attention_mask: torch.Tensor, seq_labels: torch.Tensor,
        ...
        Args:
            input_ids: [bsz, len] - tokenized input
            attention_mask: [bsz, len] - attention mask
            seq_labels: [bsz, len] - target labels
            words2pieces: [bsz, word_len, piece_len] - word-piece mapping
            ensemble: bool - not used, kept for compatibility
        
        Returns:
            Training: loss tensor
            Inference: (predictions, denoising_path)
        """
        ...
        # Aggregate word pieces to word level if needed
        if self.add_lstm:
        ...
        # Training mode
        if self.training:
            # Convert labels to bit representation
            ...
            if random() < 0.5:
                with torch.no_grad():
                    self_cond = self.model_predictions(
                        noise_bits_seq_labels, ts, features, label_mask
                    ).pred_x_start
                    self_cond.detach_()
    ...
    def prepare_targets(self, gold_seq_labels):
        ...
        Args:
            gold_seq_labels: [bsz, len] - clean labels
        
        Returns:
            diffused_labels: [bsz, len] - noisy labels
            ts: [bsz] - timestep indices
            noise: [bsz, len] - noise samples
        """
